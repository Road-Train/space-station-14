...
public sealed class ItemizePlayerSystem : EntitySystem // uppi
{
    ...
    private void AllowPickup(Entity<HumanoidAppearanceComponent> ent, ref ItemBeingPickedUpEvent args)
    {
        ...
        if (_prototypeManager.TryIndex<SpeciesPrototype>(target.Species, out var targetSpecies))
        {
        ...
        if (_prototypeManager.TryIndex<SpeciesPrototype>(source.Species, out var sourceSpecies))
        {
